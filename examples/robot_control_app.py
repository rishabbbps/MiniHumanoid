...
def connect_robot():
    ...
    try:
        ...
        st.session_state.ser = standing_pos.init_serial(
            port=PORT,
            baudrate=BAUDRATE,
...
def disconnect_robot():
    ...
    if st.session_state.ser and st.session_state.ser.is_open:
        try:
            ...
            # standing_pos.release_servos(st.session_state.ser)
            # time.sleep(0.5)
            st.session_state.ser.close()
...
def _set_initial_speeds(ser_instance):
    ...
    for channel in range(17): # Assuming 17 servos
        if not standing_pos.set_speed(ser_instance, channel, 30):
...
with main_cols[0]:
    ...
    if st.button("🧍 Stand Up / Reset", key="stand_up", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.set_positions, standing_pos.standing_position, delay=2.0,
    ...
    if st.button("🙌 Raise Hands", key="raise_hands", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.raise_hands, success_msg="Hands raised.", failure_msg="Failed to raise hands.")

    if st.button("✝️ Make Cross", key="make_cross", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.make_cross, success_msg="Cross made.", failure_msg="Failed to make cross.")

    if st.button("✌️ Make V", key="make_v", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.make_v, success_msg="V made.", failure_msg="Failed to make V.")
    ...
    if st.button("🚶 Walk", key="walk", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.walk, walk_steps, success_msg=f"Walked {walk_steps} steps.", failure_msg="Walking failed.")

    if st.button("🏃 Walk High Knees", key="walk_high", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.walk_high_knees, walk_steps, success_msg=f"Walked {walk_steps} high-knee steps.", failure_msg="High-knee walking failed.")

    if st.button("🙇 Bend Forward", key="bend_fwd", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.bend_forward, success_msg="Bent forward.", failure_msg="Failed to bend forward.")

    if st.button("🤸 Bend Backwards", key="bend_bwd", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.bend_backwards, success_msg="Bent backwards.", failure_msg="Failed to bend backwards.")

    if st.button("🥊 Boxing Jab", key="box_jab", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.perform_boxing_move, 'jab', success_msg="Performed jab.", failure_msg="Failed to jab.")

    if st.button("🤸‍♂️ Do Split", key="do_split", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.do_split, success_msg="Attempted split.", failure_msg="Failed to attempt split.")

    if st.button("🧘 Sit Down", key="sit_down", disabled=not st.session_state.connected):
        execute_robot_action(standing_pos.sit_down, success_msg="Attempted sit down.", failure_msg="Failed to attempt sit down.")
...
with main_cols[1]:
    ...
    if 'slider_values' not in st.session_state:
        st.session_state.slider_values = standing_pos.standing_position.copy() # Initialize with standing pos

    selected_channel = st.selectbox(
        "Select Servo Channel",
        options=list(standing_pos.channel_to_joint_map.keys()),
        format_func=lambda x: f"{x}: {standing_pos.channel_to_joint_map.get(x, 'Unknown')}",
        key="servo_select",
        disabled=not st.session_state.connected
    )

    if selected_channel is not None:
        joint_name = standing_pos.channel_to_joint_map.get(selected_channel, 'Unknown')
        ...
        if new_value != current_value:
            ...
            if st.session_state.connected and st.session_state.ser:
                if not standing_pos.set_target(st.session_state.ser, selected_channel, new_value):
